...
import { aboutActions, makeSelectAbout } from '../../aboutSlice'
...
function CreateAbouUs() {
  ...
  const globalDataAbout = useSelector(makeSelectAbout)
  const { isLoading, isError, isSuccess } = globalDataAbout

  // Xử lí khi có lỗi
  useEffect(() => {
    if (isError) {
      dispatch(aboutActions.clear())
      console.log('isError')
    }
  }, [isError])
  ...
  const handleUpdateDataReuqest = (data, config) => {
    if (config.field === 'document') {
  ...
  const renderDefaultFilter = item => {
    if (item.type === 'INPUT') {
      ...
      return (
        <Grid item xs={12} sm={6} key={item.field} className='mt-3'>
          <Controller
            control={control}
      ...
      return (
        <div>
          ...
          <SunEditor
            ...
            onChange={value => handleUpdateDataReuqest(value, { field: 'document' })}
